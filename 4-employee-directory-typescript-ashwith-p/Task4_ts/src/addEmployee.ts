...
var classNameMapProperty:StringArray={
...
class employeeDetails implements StringArray {
    ...
    constructor(employeeObject:StringArray,status:string){
        if(employeeObject){
...
{
    validDates();
    var details=getLocalStorage('data') as employeeDetails[];
    ...
    function checkDuplicate(className:HTMLElement){
        border_change(className);
        var dataInLocalStorage=getLocalStorage('data') as employeeDetails[];
        if(dataInLocalStorage.length){
           for(var j=0;j<dataInLocalStorage.length;j++){
                if(dataInLocalStorage[j].empNo==className.nodeValue)
                {
                    isFormValid=false;
                    ((className.parentElement) as HTMLElement).appendChild(createErrorMessage("Eployee No. Already Exists"));
                    break;
                }
            }
        }
      ...
      function validateDetails(){
        ...
        var data=getLocalStorage('data') as employeeDetails[];
        data.push(obj);
    ...
    function checkABove18(className:HTMLElement){
        ...
        if( !(currentDate.getFullYear() - birthDate.getFullYear()>=18)){
            isFormValid=false;
            ((className.parentElement)as HTMLElement).appendChild(createErrorMessage("Age is less than 18"));
        }
    }
